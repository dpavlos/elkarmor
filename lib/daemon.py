        ...
        if self._logfile:
            try:
                ...
                if e.errno == errno.EACCES:
                    log.critical('Permission denied to write to logfile `%s`',
                                 self._logfile)
                    sys.exit(1)
                if e.errno == errno.ENOENT:
                    log.critical('No such directory: `%s`',
            ...
            if self._logfile and self._logfile != '-':
                for channel in logging.getLogger().handlers:
                    ...
                    try:
                        ...
                        if channel.baseFilename == os.path.abspath(
                                self._logfile):
...
def get_daemon_option_parser(version=None, chdir='/', prog=None):
    ...
    start_group.add_option(
        ...
        help='chdir to directory DIR before starting the daemon. '
             '[default: %default]')
    start_group.add_option(
        '-k', '--umask', dest='umask', default=None,
